...
std::vector<std::string> GetConjunctionNamesFromInput(const std::vector<std::string>& input)
{
    ...
    for(const auto& line : input)
    {
        ...
        {
            conjunctionNames.push_back(line.substr(1, 2));
        }
    }

    return conjunctionNames;
}

bool ConjunctionNameInString(const std::string& line, const std::vector<std::string>& conjunctions)
{
    for(const auto& name : conjunctions)
    {
        if(line.find(name) != name.npos)
        {
            return true;
        }
    }
...
void ConnectConjunctions(const std::vector<std::string>& conjunctionConnections, Modules& modules)
{
    for(const auto& conjunctionConnection : conjunctionConnections)
    {
        std::vector<std::string> split;
        StringHandling::SplitOnString(conjunctionConnection, split, " -> ");

        std::vector<std::string> targets;
        StringHandling::SplitOnString(split[1], targets, ", ");
        const std::string& name = split[0].substr(1, 2);
        for(const auto& target : targets)
        {
            try
            {
                modules.at(target)->AddConnection(modules[name]);
            }
            catch(const std::out_of_range&)
            {}
        }
...
void FirstPart()
{
    ...
    const std::vector<std::string> conjunctionNames = GetConjunctionNamesFromInput(filePerLine);
    std::vector<std::string> conjunctionConnections;
    for(const auto& line : filePerLine)
    {
        AddToModules(line, modules);
        if(ConjunctionNameInString(line, conjunctionNames))
        {
            conjunctionConnections.push_back(line);
        }
    }

    ConnectConjunctions(conjunctionConnections, modules);
