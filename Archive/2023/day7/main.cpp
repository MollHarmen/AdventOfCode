...
const std::string CharacterSortOrder = "23456789TQKAJ";
struct
{
    bool operator()(const char& left, const char& right) const 
    {
        return CharacterSortOrder.find(left) > CharacterSortOrder.find(right); 
    }
} cardCompare;
...
std::vector<char> ReplaceJokerCards(const std::vector<char> hand)
{
    std::vector<char> replaced = hand;
    char previous = 'J';
    auto iter = hand.begin();
    while(iter != hand.end() && previous == 'J')
    {
        if(*iter != 'J')
        {
            previous = *iter;
    ...
    if(previous != 'J')
    {
        auto position = std::find(replaced.begin(), replaced.end(), 'J');
        while(position != replaced.end())
        {
            *position = previous;
            position = std::find(replaced.begin(), replaced.end(), 'J');
        }
    }
...
struct Hand
{
    ...
    , mBid(bid)
    {
        ...
        mSortedHand = ReplaceJokerCards(mSortedHand);
    ...
    void Print() const
    {
        ...
        const std::string sortedHand(mSortedHand.begin(), mSortedHand.end());
        std::cout << hand << " " << sortedHand << " " << mPattern << " " << mBid;
    }
    ...
    std::string GetHandPattern() const
    {
        ...
        {
            if(*cardIterator == 'J' || *cardIterator == previousChar)
            {
